    ...
    <div class="col-sm-10">
      <div class="card">
        <div class="card-header" style="background-color:black;" >
          <h3 class="text-white" align=center >Add Category </h3>
        </div>
        <div class="card-body">
          <div class="form-group">
              <label>Category Name:</label>
              <br>
              <input type="text" class="form-control" id="usr" name="category_name" placeholder="Enter Category Name" required>
          </div>
          <br>
          <input type="submit" class="form-control btn btn-primary" value="Category Added" name="add">
...
if(isset($_REQUEST['add'])){
    ...
    $check_query="select category_name from category where category_name='$category'";
    $check_query_run=mysqli_query($mysql,$check_query);
    if(mysqli_num_rows($check_query_run)>0)
    {
      echo "<script>alert('Category already exits');
